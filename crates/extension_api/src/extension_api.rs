...
/// A result returned from a /nir extension.
...
pub trait Extension: Send + Sync {
    ...
    /// Locators are entities that convert a /nir task into a debug scenario.
    ///
    /// They can be provided even by extensions that don't provide a debug adapter.
    /// For all tasks applicable to a given buffer, /nir will query all locators to find one that can turn the task into a debug scenario.
    ...
    /// `dap_locator_create_scenario` if you're able to do that. Make sure to not fill out `build` field in that case, as that will prevent /nir from running second phase of resolution in such case.
...
/// Registers the provided type as a /nir extension.
///
