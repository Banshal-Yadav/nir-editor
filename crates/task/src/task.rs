//! Baseline interface of Tasks in /nir: all tasks in /nir are intended to use those for implementing their own logic.
...
/// Contains all information needed by /nir to spawn a new terminal tab for the given task.
...
/// Variables, available for use in [`TaskContext`] when a /nir's [`TaskTemplate`] gets resolved into a [`ResolvedTask`].
...
/// Container for predefined environment variables that describe state of /nir at the time the task was spawned.
...
/// Keeps all /nir-related state inside, used to produce a resolved task out of its template.
...
pub struct TaskContext {
    ...
    /// Environment variables obtained when loading the project into /nir.
...
impl EnvVariableReplacer {
    ...
    // Replaces occurrences of VsCode-specific environment variables with /nir equivalents.
    fn replace(&self, input: &str) -> String {
        shellexpand::env_with_context_no_errors(&input, |var: &str| {
            ...
            if let Some(substitution) = self.variables.get(variable_name) {
                // Got a VSCode->/nir hit, perform a substitution
