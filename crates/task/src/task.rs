//! Baseline interface of Tasks in /void: all tasks in /void are intended to use those for implementing their own logic.
...
/// Contains all information needed by /void to spawn a new terminal tab for the given task.
...
/// Variables, available for use in [`TaskContext`] when a /void's [`TaskTemplate`] gets resolved into a [`ResolvedTask`].
...
/// Container for predefined environment variables that describe state of /void at the time the task was spawned.
...
/// Keeps all /void-related state inside, used to produce a resolved task out of its template.
...
pub struct TaskContext {
    ...
    /// Environment variables obtained when loading the project into /void.
...
impl EnvVariableReplacer {
    ...
    // Replaces occurrences of VsCode-specific environment variables with /void equivalents.
    fn replace(&self, input: &str) -> String {
        shellexpand::env_with_context_no_errors(&input, |var: &str| {
            ...
            if let Some(substitution) = self.variables.get(variable_name) {
                // Got a VSCode->/void hit, perform a substitution
