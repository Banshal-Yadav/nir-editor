...
use workspace::{
    ...
    SidebarSide, Toast, Workspace, notifications::NotificationId,
};
...
impl Sidebar {
    ...
    fn rebuild_contents(&mut self, cx: &App) {
        ...
        let resolve_agent_icon = |agent_id: &AgentId| -> (IconName, Option<SharedString>) {
            ...
            let icon = match agent {
                Agent::NativeAgent => IconName::VoidAgent,
    ...
    fn entry_shapes<'a>(
        ...
        self.contents.entries.iter().map(move |entry| match entry {
            ...
            ListEntry::SectionHeader { label, .. } => EntryShape::SectionHeader(label.clone()),
        })
    }
    ...
    fn render_acp_import_onboarding(
        ...
        render_import_onboarding_banner(
            ...
            "Import threads from agents like Claude Agent, Codex, and more, whether started in /nir or another client.",
            if verbose_labels {
