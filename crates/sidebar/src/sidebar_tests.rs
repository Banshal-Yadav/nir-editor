...
/// same `RemoteConnectionIdentity`, matching how /nir treats multiple
...
async fn test_remote_archive_thread_with_active_connection(
    ...
    // linked worktree across two remote projects the way /nir does in
...
async fn test_pinned_threads(cx: &mut TestAppContext) {
    ...
    assert_eq!(
        visible_entries_as_strings(&sidebar, cx),
        vec!["v [my-project]", "  Thread C", "  Thread B", "  Thread A",]
    );
