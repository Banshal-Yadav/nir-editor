...
const ZED_REPL_DOCUMENTATION: &str = "https://github.com/Banshal-Yadav/nir-editor/wiki";
...
impl QuickActionBar {
    ...
    pub fn render_repl_setup(&self, language: &str, cx: &mut Context<Self>) -> Option<AnyElement> {
        let tooltip: SharedString = SharedString::from(format!("Setup /nir REPL for {}", language));
        Some(
            h_flex()
