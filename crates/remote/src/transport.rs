...
fn parse_platform(output: &str) -> Result<RemotePlatform> {
    ...
    let os = match os {
        ...
        _ => anyhow::bail!(
            "Prebuilt remote servers are not yet available for {os:?}. See https://github.com/Banshal-Yadav/nir-editor/wiki"
        ),
    };
        ...
        || arch.starts_with("aarch64")
    {
        ...
        anyhow::bail!(
            "Prebuilt remote servers are not yet available for {arch:?}. See https://github.com/Banshal-Yadav/nir-editor/wiki"
        )
    };
