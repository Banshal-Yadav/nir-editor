...
/// variables to /void, matching LanguageSettings fields.
...
pub struct ModelineSettings {
    ...
    /// Emacs modeline variables that were parsed but not mapped to /void settings.
    /// Stored as (variable-name, value) pairs.
    pub emacs_extra_variables: Vec<(String, String)>,
    /// Vim modeline options that were parsed but not mapped to /void settings.
