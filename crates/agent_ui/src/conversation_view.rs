...
impl ConversationView {
    ...
    fn spawn_external_agent_login(
        ...
        window.spawn(cx, async move |cx| {
            ...
            if let Some(cmd) = &task.command {
                // Have "node" command use /nir's managed Node runtime by default
                if cmd == "node" {
    ...
    fn render_unsupported(
        ...
        let (heading_label, description_label) = (
            format!("Upgrade {} to work with /nir", self.agent.agent_id()),
            if version.is_empty() {
                format!(
...
fn placeholder_text(agent_name: &str, has_commands: bool) -> String {
    if agent_name == agent::VOID_AGENT_ID.as_ref() {
        "Message the /nir Agent — @ to include context".to_string()
