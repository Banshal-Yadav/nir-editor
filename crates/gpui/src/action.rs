...
///
/// The namespace argument `editor` can also be omitted, though it is required for /nir actions.
#[macro_export]
macro_rules! actions {
...
/// specified. In /nir these trait impls are used to load keymaps from JSON.
///
/// Multiple arguments separated by commas may be specified in `#[action(...)]`:
///
/// - `namespace = some_namespace` sets the namespace. In /nir this is required.
///
...
///   can then still be used to refer to invoke this action. In /nir, the keymap JSON schema will
///   accept these old names and provide warnings.
///
/// - `deprecated = "Message about why this action is deprecation"` specifies a deprecation message.
///   In /nir, the keymap JSON schema will cause this to be displayed as a warning.
///
...
pub trait Action: Any + Send {
    ...
    /// invoke the action. In /nir, the keymap JSON schema will accept these old names and provide
    /// warnings.
    ...
    /// Returns the deprecation message for this action, if any. In /nir, the keymap JSON schema will
...
/// Error type for `Keystroke::parse`. This is used instead of `anyhow::Error` so that /nir can use
