...
impl PythonDebugAdapter {
    ...
    async fn request_args(
        ...
        if let Ok(console) = configuration.dot_get_mut("console") {
            // Use built-in /nir terminal if user did not explicitly provide a setting for console.
            if console.is_null() {
    ...
    async fn get_binary(
        ...
        let base_paths = ["cwd", "program", "module"]
            ...
            .chain(
                ...
                // (Which should always be the case because /nir defaults to the cwd worktree root)
