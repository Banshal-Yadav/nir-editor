...
Headless CLI binary for running /void's agent in evaluation/benchmark
...
Uses the same `NativeAgent` + `AcpThread` pipeline as the production /void
...
The `void_eval/` directory contains a Python package that
...
```
pip install -e crates/eval_cli/void_eval/
```
