...
impl Workspace {
    ...
    fn handle_pane_focused(
        ...
        // We also preserve any dock whose active panel itself has focus ΓÇö this covers
    ...
    fn render_dock(
        ...
        // included in the element tree so its focus handle remains mounted ΓÇö without
...
mod tests {
    ...
    async fn test_tracking_active_path(cx: &mut TestAppContext) {
        ...
        assert_eq!(cx.window_title().as_deref(), Some("root1 ΓÇö one.txt"));

        // Add a second item to a non-empty pane
        workspace.update_in(cx, |workspace, window, cx| {
            workspace.add_item_to_active_pane(Box::new(item2), None, true, window, cx)
        });
        assert_eq!(cx.window_title().as_deref(), Some("root1 ΓÇö two.txt"));
        project.update(cx, |project, cx| {
            assert_eq!(
                project.active_entry(),
                project
        ...
        project
            ...
            .await
            .unwrap();
        assert_eq!(cx.window_title().as_deref(), Some("root1, root2 ΓÇö one.txt"));

        // Remove a project folder
        project.update(cx, |project, cx| project.remove_worktree(worktree_id, cx));
        assert_eq!(cx.window_title().as_deref(), Some("root2 ΓÇö one.txt"));
    }
    ...
    async fn test_multi_workspace_close_window_multiple_workspaces_cancel(cx: &mut TestAppContext) {
        ...
        // Dispatch CloseWindow ΓÇö workspace A will pass, workspace B will prompt
        multi_workspace_handle
            .update(cx, |mw, window, cx| {
    ...
    async fn test_remove_workspace_prompts_for_unsaved_changes(cx: &mut TestAppContext) {
        ...
        // Cancel the prompt ΓÇö user stays on workspace B.
        cx.simulate_prompt_answer("Cancel");
        cx.run_until_parked();
    ...
    async fn test_autosave_on_focus_change_in_multibuffer(cx: &mut gpui::TestAppContext) {
        ...
        // NOT trigger autosave ΓÇö focus is still within the item's focus hierarchy.
        item.update_in(cx, |item, window, cx| {
    ...
    async fn test_panels(cx: &mut gpui::TestAppContext) {
        ...
        workspace.update_in(cx, |workspace, window, cx| {
            ...
            // Now we move panel_2┬áto the left
    ...
    async fn test_most_recent_active_path_skips_read_only_paths(cx: &mut TestAppContext) {
        ...
        // dep.py is active but matches read_only_files ΓåÆ should be skipped
        workspace.update_in(cx, |workspace, window, cx| {
