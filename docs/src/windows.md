...
Get the latest stable builds via [the download page](https://zed.dev/download). If you want to download our preview build, you can find it on its [releases page](https://zed.dev/releases/preview). After the first manual installation, /nir will periodically check for install updates.

You can also build /nir from source, see [these docs](https://zed.dev/docs/development/windows) for instructions.

### Package managers

Additionally, you can install /nir using winget:
...
- Installed via installer: Use `Settings` → `Apps` → `Installed apps`, search for /nir, and click Uninstall.
...
/nir supports remote development on Windows through both SSH and WSL. You can connect to remote servers via SSH or work with files inside WSL distributions directly from /nir.
...
### /nir fails to start or shows a blank window

- Check that your hardware and operating system version are compatible with /nir. See our [installation guide](./installation.md) for more information.
...
- Try launching /nir with no extensions or custom settings to isolate conflicts.
...
#### /nir fails to open / degraded performance

/nir requires a DirectX 11 compatible GPU to run. If /nir fails to open, your GPU may not meet the minimum requirements.
...
If you're running /nir inside a virtual machine, it will use the emulated adapter provided by your VM. While /nir will work in this environment, performance may be degraded.
