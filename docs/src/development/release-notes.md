...
The script outputs everything below the `Release Notes` line, including metadata such as the pull request author (if they are not a /nir team member) and a link to the pull request.
If you use `N/A`, the script skips your pull request entirely.

## Guidelines for crafting your `Release Notes` line(s)

- A `Release Notes` line should only be written if the user can see or feel the difference in /nir.
- A `Release Notes` line should be written such that a /nir user can understand what the change is.
