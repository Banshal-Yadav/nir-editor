# /nir's Feature Development Process

This is for moderate-to-large features — new UI, behavior changes, or work that cuts across multiple parts of /nir. Small keybindings or settings tweaks don't need all of this.
...
- **Is there prior art?** If it's in VS Code, JetBrains, Neovim, or a wildly popular plugin, that's a strong signal. If the idea is more novel, name what it's based on — "This is X, adapted for /nir's multi-buffers" is far more useful than "I think this would be cool."
...
**Decisions:**
We have to decide whether to use the git CLI or a git library. /nir uses a git library but its blame implementation is too slow for a code editor, so we should use the CLI's porcelain interface.
...
- **Security.** How does this feature interact with Workspace Trust? Does it open new attack surfaces in /nir?
