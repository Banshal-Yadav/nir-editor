...
description: "Performance profiling and optimization for /nir development."
---

How to use our internal tools to profile and keep /nir fast.
...
Then either compile /nir with `ZTRACING=1 cargo r --features tracy --release`. The release build is optional but highly recommended as like every program /nir's performance characteristics change dramatically with optimizations. You do not want to chase slowdowns that do not exist in release.
